    ...
    return (
        ...
        dark:bg-dark dark:border-light xs:p-4 '>
            ...
            <Link href={link} target="_blank"
                ...
                <FramerImage src={img} alt={title} className="w-full h-auto sm:w-full md:w-full"
                    whileHover={{ scale: 1.05 }}
                    transition={{ duration: 0.2 }} />
            </Link>
            <div className='w-full h-full flex flex-col items-start justify-between mt-4'>
                <span className='text-primary font-medium text-xl dark:text-primaryDark lg:text-lg md:text-base'>{type}</span>
                <Link href={link} target="_blank" className='hover:underline underline-offset-2'> <h2 className='my-2 w-full text-left text-3xl font-bold dark:text-light lg:text-2xl'>{title}</h2></Link>
                <p className='my-2 font-medium text-dark dark:text-light sm:text-sm'>{summary}</p>
                <div className='mt-2 flex items-center '>
                    <Link href={github} target="_blank" className='w-10  md:w-6'> <GithubIcon /> </Link>
                    <Link href={link} target="_blank"
                        className='ml-4 rounded-lg bg-dark text-light p-2 px-6 text-lg font-semibold  dark:bg-light dark:text-dark md:text-base'>Visit</Link>
                </div>
            </div>
        </article>
    )
